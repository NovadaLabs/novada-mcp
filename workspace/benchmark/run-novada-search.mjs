...
const TARGETS = {
  ...
  T3: { type: "search", query: "bitcoin price 2025",                                           label: "Search — Financial" },
  T4: { type: "search", query: "agent memory system AI",                                       label: "Search — AI" },
  T5: { type: "crawl",  url: "https://docs.python.org/3/library/collections.html", maxPages: 3, label: "Crawl (Python Docs)" },
  T6: { type: "scrape_structured", url: "amazon", asin: "B09B96TG33",                         label: "Structured Data (Amazon)" },
};

const ROUNDS = 10; // measured rounds per (competitor × category)
...
async function novadaStructured(asin) {
  // Try Amazon product via scraper API
  const form = new URLSearchParams();
  form.append("scraper_name", "amazon.com");
  form.append("scraper_id", "amazon_product");
  form.append("scraper_errors", "true");
  form.append("is_auto_push", "false");
  ...
  const body = submit.data;
  if (body.code !== 0) throw new Error(`Novada Amazon submit error (${body.code}): ${body.msg}`);

  const inner = body.data;
  const taskId = inner?.task_id ?? inner?.data?.task_id;
  if (!taskId) throw new Error(`No task_id for Amazon: ${JSON.stringify(body).slice(0, 200)}`);
  ...
  while (Date.now() < deadline) {
    ...
    if (Array.isArray(dlBody) && dlBody.length > 0) {
      ...
      return { text, raw: item };
    }
    if (dlBody?.code === 27202) { await sleep(2000); continue; }
    const text = JSON.stringify(dlBody).slice(0, 5000);
    return { text, raw: dlBody };
  }
  throw new Error("Novada Amazon task timed out after 85s");
}
...
async function oxylabsAmazon(asin) {
  const resp = await axios.post(
    "https://realtime.oxylabs.io/v1/queries",
    { source: "amazon_product", query: asin, domain: "com" },
    {
      headers: { Authorization: `Basic ${OXY_AUTH}`, "Content-Type": "application/json" },
      timeout: 55000,
    }
...
const COMPETITORS = ["novada"];
const CATEGORIES = ["T3", "T4"];
...
async function runBenchmark() {
  console.log("╔══════════════════════════════════════════════════════╗");
  console.log("║   MCP Web Scraping Benchmark v1.0 — 2026-05-21       ║");
  console.log("╚══════════════════════════════════════════════════════╝\n");
